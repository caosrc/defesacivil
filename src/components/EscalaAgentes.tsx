...
function salvarDados(data: EscalaData) {
  ...
  // Persistência remota via API do servidor (não bloqueia o salvamento local)
  fetch('/api/escala', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  }).catch(err => console.warn('Falha ao salvar escala remotamente:', err?.message ?? err))
}
...
async function carregarDadosRemoto(): Promise<EscalaData | null> {
  try {
    ...
    const p = json as Partial<EscalaData> & Record<string, unknown>
    return {
