...
- **Banco de horas — coluna "dias de folga"**: abaixo das horas de cada agente é exibido `total ÷ horas/folga` = dias de folga disponíveis. **Horas por folga variam por agente** (função `horasPorFolga`): Talita e Cristiane = 4h, Sócrates = 6h, demais = 8h. O divisor é refletido no painel do Moisés, no tooltip de detalhe, no rodapé descritivo e na exportação Excel por agente.
- **Botão "💾 Salvar horas no banco de dados"** no painel de horas extras (Talita/Cristiane/Sócrates): substitui o antigo `+ Adicionar horas`. Faz `salvarDadosAsync` (await do `supabase.upsert`) e mostra feedback visual verde "✅ Salvo no banco de dados" ou amarelo "⚠️ Salvo localmente. Falha ao enviar ao servidor" por 4s.
- **SOS Crítico** (`src/sos.ts`, `src/components/BotaoSos.tsx`, `src/components/SosOverlay.tsx`): botão flutuante 🆘 sempre visível (canto inferior direito) que dispara em **segurar 1,5s** (com barra de progresso para evitar acidentes). Também escuta as teclas físicas `AudioVolumeUp`/`AudioVolumeDown` para abrir o painel de confirmação. Ao disparar, captura GPS, % de bateria (`navigator.getBattery`) e grava 10s de áudio (`MediaRecorder`), enviando broadcast pelo canal Supabase Realtime `sos-alerta-defesa-civil` (eventos `sos`, `sos-audio`, `sos-cancelar`). Todos os agentes logados recebem `SosOverlay`: tela vermelha pulsante em cima de tudo (z-index 99999), card piscante com nome do agente, tempo decorrido, GPS, bateria, player de áudio, sirene Web Audio (oscillator alternando 600↔1100 Hz), vibração (`navigator.vibrate`), botão **🗺️ Traçar rota de resgate** (Google Maps Directions) e **Dispensar alerta** (broadcast `sos-cancelar`). Auto-dismiss em 1h via `setTimeout`.
